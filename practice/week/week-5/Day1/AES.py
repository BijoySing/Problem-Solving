# implement aes encryption and decryption
s_box = [
    
]

s_box_inv = [
    
]


# AES Round Constants
r_con = [
    [0x00, 0x00, 0x00, 0x00],
    [0x01, 0x00, 0x00, 0x00],
    [0x02, 0x00, 0x00, 0x00],
    [0x04, 0x00, 0x00, 0x00],
    [0x08, 0x00, 0x00, 0x00],
    [0x10, 0x00, 0x00, 0x00],
    [0x20, 0x00, 0x00, 0x00],
    [0x40, 0x00, 0x00, 0x00],
    [0x80, 0x00, 0x00, 0x00],
    [0x1b, 0x00, 0x00, 0x00],
    [0x36, 0x00, 0x00, 0x00]        
]

def generate_mul_tables():
    mul2 = [0] * 256
    mul3 = [0] * 256

    for i in range(256):
        mul2[i] = (i << 1) ^ (0x11B if (i & 0x80) else 0)
        mul3[i] = mul2[i] ^ i

    return mul2, mul3
mul2, mul3 = generate_mul_tables()



def print_state(state):
    # state = transpose_state(state)
    for row in state:
        hex_row = [format(byte, '02X') for byte in row]
        print(hex_row)
def transpose_state(state):
    return [[state[j][i] for j in range(4)] for i in range(4)]



def add_round_key(state, key):
    # return [[state[i][j] ^ key[i][j] for j in range(4)] for i in range(4)]
    for i in range(4):
        for j in range(4):
            state[i][j] ^= key[j][i]


def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = s_box[state[i][j]]


def shift_rows(state):
    # state=transpose_state(state)
    for i in range(1, 4):
        state[i] = state[i][i:] + state[i][:i]
    # state=transpose_state(state)
    return state


#implement mix_columns using gf theorem
def mix_columns(state):
    for i in range(4):
        col = [state[j][i] for j in range(4)]
        temp = []
        for row in range(4):
            a = col[row]
            b = col[(row + 1) % 4]
            c = col[(row + 2) % 4]
            d = col[(row + 3) % 4]
            result = gf_multiply(a, 2) ^ gf_multiply(b, 3) ^ gf_multiply(c, 1) ^ gf_multiply(d, 1)
            temp.append(result)
        for j in range(4):
            state[j][i] = temp[j]


def key_expansion(key):
    round_keys = [key[i:i+4] for i in range(0, len(key), 4)]
    for i in range(4, 44):
        word = round_keys[i-1]
        if i % 4 == 0:
            word = [word[1], word[2], word[3], word[0]]
            word = [s_box[byte] for byte in word]
            word[0] ^= r_con[i//4][0]
        round_keys.append([word[j] ^ round_keys[i-4][j] for j in range(4)])
    return round_keys

# def aes_encrypt(plaintext, key):
#     state = [plaintext[i:i + 4] for i in range(0, len(plaintext), 4)]
#     state=transpose_state(state)
#     round_keys = key_expansion(key)
#     add_round_key(state, round_keys[:4])
#     print("After add round key")
#     print_state(state)
#     for i in range(1, 10):
#         print("Round", i)
#         sub_bytes(state)
#         print("After sub bytes")
#         print_state(state)
#         state=shift_rows(state)
#         print("After shift rows")
#         print_state(state)
#         mix_columns(state)
#         print("After mix columns")
#         print_state(state)
#         add_round_key(state, round_keys[4*i:4*(i+1)])
#         print("After add round key")
#         print_state(state)
#     sub_bytes(state)
#     state=shift_rows(state)
#     add_round_key(state, round_keys[40:])
#     print("Final State")
#     print_state(state)
#     state=transpose_state(state)
    
#     return [state[i][j] for i in range(4) for j in range(4)]

#encrypt aes block using aes encryption each block is 16 bytes


# def aes_encrypt(plaintext, key): that will divide the plaintext into blocks of 16 bytes and encrypt each block using aes encryption   
def aes_encrypt(plaintext, key):
    ciphertext = []
    print(len(plaintext))
    for i in range(0, len(plaintext), 16):
        block = plaintext[i:i + 16]
        state = [block[i:i + 4] for i in range(0, len(block), 4)]
        # # print_state(state)
        # print("\n")
        state=transpose_state(state)
        round_keys = key_expansion(key)
        add_round_key(state, round_keys[:4])
        for i in range(1, 10):
            sub_bytes(state)
            state=shift_rows(state)
            mix_columns(state)
            add_round_key(state, round_keys[4*i:4*(i+1)])
        sub_bytes(state)
        state=shift_rows(state)
        add_round_key(state, round_keys[40:])
        state=transpose_state(state)
        ciphertext.extend([state[i][j] for i in range(4) for j in range(4)])
    return ciphertext

# impelement aes decryption
def inv_sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = s_box_inv[state[i][j]]

def inv_shift_rows(state):
    # state=transpose_state(state)
    for i in range(1, 4):
        state[i] = state[i][-i:] + state[i][:-i]
    
def gf_multiply(a, b):
    result = 0
    while a and b:
        if b & 1:
            result ^= a
        a = mul2[a]
        b >>= 1
    return result

inv_mix_column_matrix = [
    [0x0e, 0x0b, 0x0d, 0x09],
    [0x09, 0x0e, 0x0b, 0x0d],
    [0x0d, 0x09, 0x0e, 0x0b],
    [0x0b, 0x0d, 0x09, 0x0e]
]


# implement inv_mix_columns using gf theorem
def inv_mix_columns(state):
    for i in range(4):
        col = [state[j][i] for j in range(4)]
        temp = []
        for row in range(4):
            a = col[row]
            b = col[(row + 1) % 4]
            c = col[(row + 2) % 4]
            d = col[(row + 3) % 4]
            result = gf_multiply(a, 0x0e) ^ gf_multiply(b, 0x0b) ^ gf_multiply(c, 0x0d) ^ gf_multiply(d, 0x09)
            temp.append(result)
        for j in range(4):
            state[j][i] = temp[j]
    


#implement aes decryption using aes decryption each block is 16 bytes 
def aes_decrypt(ciphertext, key):
    plaintext = []
    for i in range(0, len(ciphertext), 16):
        block = ciphertext[i:i + 16]
        state = [block[i:i + 4] for i in range(0, len(block), 4)]
        state=transpose_state(state)
        round_keys = key_expansion(key)
        add_round_key(state, round_keys[40:])
        inv_shift_rows(state)
        inv_sub_bytes(state)
        for i in range(9, 0, -1):
            add_round_key(state, round_keys[4*i:4*(i+1)])
            inv_mix_columns(state)
            inv_shift_rows(state)
            inv_sub_bytes(state)
        add_round_key(state, round_keys[:4])
        state=transpose_state(state)
        plaintext.extend([state[i][j] for i in range(4) for j in range(4)])
    return plaintext
    


def main():
    while True:
        # text = "IsTheirCarnivalSuccessful"
        # text = "Two One Nine Two"
        # keyy = "Thats my Kung Fu"
        # keyy = "SUST CSE19 Batch"
        text = input("Enter the text to encrypt: ")
        keyy = input("Enter the key: ")


        length = len(text)
        plaintext = text
        if len(plaintext) % 16 != 0:
            padding_length = 16 - (len(plaintext) % 16)
            plaintext += '0' * padding_length  # Add padding directly to the string
        plaintext = plaintext.encode('utf-8')
        plaintext = [byte for byte in plaintext]  # Convert bytes to list of integers
        plaintext = [byte & 0xFF for byte in plaintext]  # Ensure each byte is within the range [0, 255]
        key = keyy.encode('utf-8')
        if len(key) != 16:
            # Add padding
            key += b'\0' * (16 - len(key))
        #took 16 bytes of key
        key = key[:16]
        key = [byte for byte in key]  # Convert bytes to list of integers
        key = [byte & 0xFF for byte in key]  # Ensure each byte is within the range [0, 255]

        print("Plaintext:", ' '.join(format(byte, '02X') for byte in plaintext))
        print("Key:", ' '.join(format(byte, '02X') for byte in key))
        ciphertext = aes_encrypt(plaintext, key)
        print("Plaintext:", ' '.join(format(byte, '02X') for byte in plaintext))
        print("Ciphertext:", ' '.join(format(byte, '02X') for byte in ciphertext))
        decrypted = aes_decrypt(ciphertext, key)
        print("Decrypted:", ' '.join(format(byte, '02X') for byte in decrypted))
        decrypted = bytes(decrypted)
        decrypted = decrypted.rstrip(b'\0').decode('utf-8')
        #print as lentgh of text
        print("Decrypted:", decrypted[:length])
        # print("Decrypted:", decrypted)
        # Ask the user if they want to continue
        choice = input("Do you want to continue? (yes/no): ")
        if choice.lower() != "yes":
            break


if __name__ == "__main__":
    main()
